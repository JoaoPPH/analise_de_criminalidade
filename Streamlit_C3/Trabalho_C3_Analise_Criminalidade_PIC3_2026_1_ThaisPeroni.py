...
with abas[0]:
    ...
    st.markdown("#### Filtros da Análise Geral")

    col_uf_t, col_ano_t = st.columns(2)

    with col_uf_t:
        lista_uf_t = sorted(df['uf'].dropna().unique())
        ufs_selecionadas_t = st.multiselect(
            "Filtrar por Estado (UF):",
            options=lista_uf_t,
            key="multiselect_uf_t"
        )

    with col_ano_t:
        lista_anos_t = sorted(df[df['Ano'] < 2026]['Ano'].dropna().unique())
        anos_selecionados_t = st.multiselect(
            "Filtrar por Ano:",
            options=lista_anos_t,
            key="multiselect_ano_t"
        )

    df_filtrado_t = df[df['Ano'] < 2026].copy()

    if ufs_selecionadas_t:
        df_filtrado_t = df_filtrado_t[df_filtrado_t['uf'].isin(ufs_selecionadas_t)]

    if anos_selecionados_t:
        df_filtrado_t = df_filtrado_t[df_filtrado_t['Ano'].isin(anos_selecionados_t)]
    ...
    subabas_t = st.tabs([
        "Visão Geral",
        "Crimes por Estado",
        "Crimes por Município",
        "Crime mais frequente por Estado",
        "Evolução",
    ...
    with subabas_t[0]:
        st.subheader("Visão Geral da Base Criminal")

        total_ocorrencias = int(df_filtrado_t["total"].sum())

        total_estados = df_filtrado_t["uf"].nunique()

        total_municipios = df_filtrado_t["municipio"].nunique()

        total_eventos = df_filtrado_t["evento"].nunique()

        c1, c2, c3, c4 = st.columns(4)

        c1.metric("Total de Ocorrências", f"{total_ocorrencias:,}")

        c2.metric("Estados", total_estados)

        c3.metric("Municípios", total_municipios)

        c4.metric("Tipos de Evento", total_eventos)
        
        st.divider()

        st.markdown("#### Top 10 Estados com Mais Ocorrências")

        df_top_estados = (
            df_filtrado_t.groupby("uf")["total"]
            .sum()
            .reset_index()
        ...
        fig_top_estados = px.bar(
            ...
            x="uf",
            y="total",
            title="Top 10 Estados com Maior Número de Ocorrências",
            labels={"uf": "Estado (UF)", "total": "Total de Ocorrências"},
            text="total",
            color="total",
        ...
        st.markdown("#### Dados dos Estados com Mais Ocorrências")

        st.dataframe(
            df_top_estados.rename(columns={
                "uf": "Estado",
    ...
    with subabas_t[1]:
        ...
        fig_crimes_estado = px.bar(
            df_crimes_estado,
            x="uf",
            y="total",
            title="Total de Ocorrências por Estado",
            labels={"uf": "Estado (UF)", "total": "Total de Ocorrências"},
            text="total",
            color="total",
        ...
        fig_crimes_estado.update_traces(
            texttemplate="%{text:,.0f}",
            textposition="outside"
        )

        fig_crimes_estado.update_layout(
            xaxis_title="Estado",
            yaxis_title="Total de Ocorrências"
        )

        st.plotly_chart(fig_crimes_estado, use_container_width=True)

        st.divider()

        st.markdown("#### Tabela de Ocorrências por Estado")

        st.dataframe(
            df_crimes_estado.rename(columns={
                "uf": "Estado",
                "total": "Total de Ocorrências"
            }).style.format({
                "Total de Ocorrências": "{:,.0f}"
    ...
    # Sub-aba 3: Crimes por Município
    with subabas_t[2]:
        st.subheader("Top 20 Municípios com Mais Ocorrências")

        df_municipios = (
            df_filtrado_t[
                (df_filtrado_t["municipio"].notna()) &
                (df_filtrado_t["municipio"] != "NÃO INFORMADO")
    ]
        ...
        fig_municipios = px.bar(
            ...
            y="municipio",
            orientation="h",
            title="Top 20 Municípios com Maior Número de Ocorrências",
            labels={
                "municipio": "Município",
                "total": "Total de Ocorrências"
            },
        ...
        fig_municipios.update_yaxes(autorange="reversed")

        st.plotly_chart(fig_municipios, use_container_width=True)

        st.divider()

        st.markdown("#### Tabela dos Municípios")
    ...
    # Sub-aba 4: Crime mais frequente por Estado
    with subabas_t[3]:
        st.subheader("Crime Mais Frequente em Cada Estado")
        ...
        df_crimes_validos = df_filtrado_t[
            ~df_filtrado_t["evento"].isin(eventos_remover)
        ].copy()

        df_estado_evento = (
            df_crimes_validos.groupby(["uf", "evento"])["total"]
            .sum()
            .reset_index()
        )

        df_crime_mais_frequente = (
            df_estado_evento.sort_values(["uf", "total"], ascending=[True, False])
            .groupby("uf")
            .head(1)
            .sort_values(by="total", ascending=False)
        )

        fig_crime_estado = px.bar(
            df_crime_mais_frequente,
            x="uf",
            y="total",
            color="evento",
            title="Crime Mais Frequente por Estado",
            labels={
                "uf": "Estado (UF)",
                "total": "Total de Ocorrências",
                "evento": "Crime mais frequente"
            },
            text="total"
        )

        fig_crime_estado.update_traces(
            texttemplate="%{text:,.0f}",
            textposition="outside"
        )

        st.plotly_chart(fig_crime_estado, use_container_width=True)

        st.divider()

        st.markdown("#### Tabela do Crime Mais Frequente por Estado")

        st.dataframe(
            df_crime_mais_frequente.rename(columns={
                "uf": "Estado",
                "evento": "Crime mais frequente",
                "total": "Total de Ocorrências"
            }).style.format({
    ...
    # Sub-aba 5: Evolução
    with subabas_t[4]:
        st.subheader("Evolução Histórica das Ocorrências Criminais")

        df_historico = (
            df_filtrado_t.groupby("Ano")["total"]
            .sum()
            .reset_index()
            .sort_values("Ano")
        )
        ...
        fig_historico = px.line(
            ...
            x="Ano",
            y="total",
            title="Evolução do Número de Ocorrências ao Longo dos Anos",
            markers=True
        )
    ...
    # Sub-aba 6: Regressão Linear e Previsões
    with subabas_t[5]:
        st.subheader("Regressão Linear e Previsões para o Sudeste")

        st.write(
            "Esta análise utiliza Regressão Linear para observar a tendência histórica "
            "de crimes específicos nos estados do Sudeste e projetar valores futuros."
        )

        opcoes_regressao = {
            "Espírito Santo - Tráfico de drogas": ("ES", "Tráfico de drogas"),
            "São Paulo - Furto de veículo": ("SP", "Furto de veículo"),
            "Rio de Janeiro - Roubo de veículo": ("RJ", "Roubo de veículo"),
            "Minas Gerais - Furto de veículo": ("MG", "Furto de veículo")
        }

        escolha_regressao = st.selectbox(
            "Selecione a análise:",
            options=list(opcoes_regressao.keys()),
            key="select_regressao_thais"
        )

        uf_escolhida, crime_escolhido = opcoes_regressao[escolha_regressao]

        df_regressao = df[
            (df["uf"] == uf_escolhida) &
            (df["evento"] == crime_escolhido) &
            (df["Ano"] < 2026)
        ].copy()

        df_regressao_ano = (
            df_regressao.groupby("Ano")["total"]
            .sum()
            .reset_index()
            .sort_values("Ano")
        )

        if len(df_regressao_ano) < 2:
            st.warning("Não há dados suficientes para calcular a regressão linear.")
        else:
            X = df_regressao_ano[["Ano"]]
            y = df_regressao_ano["total"]

            modelo = LinearRegression()
            modelo.fit(X, y)

            df_regressao_ano["Reta Ajustada"] = modelo.predict(X)

            anos_futuros = pd.DataFrame({"Ano": [2027, 2028, 2029]})
            ...
            with c1:
                st.markdown("**Estado analisado**")
                st.write(uf_escolhida)

            with c2:
                st.markdown("**Crime analisado**")
                st.write(opcoes_regressao[escolha_regressao][1])
            ...
            st.metric("Coeficiente da Regressão", f"{coeficiente:,.2f}")
            ...
            fig_regressao.add_trace(go.Scatter(
                x=df_regressao_ano["Ano"],
                y=df_regressao_ano["Reta Ajustada"],
                mode="lines",
                name="Reta Ajustada",
                line=dict(dash="dash")
            ))
            ...
            fig_regressao.add_trace(go.Scatter(
                ...
                y=previsoes,
                mode="lines+markers",
                name="Previsão 2027-2029",
                line=dict(dash="dot")
            ))

            fig_regressao.update_layout(
                title=f"Regressão Linear - {crime_escolhido} em {uf_escolhida}",
                xaxis_title="Ano",
                yaxis_title="Total de Ocorrências",
                xaxis=dict(tickmode="linear", dtick=1),
            ...
            fig_regressao.update_yaxes(rangemode="tozero")
            ...
            st.markdown("#### Previsões para os Próximos Anos")

            st.dataframe(
                df_previsoes.style.format({
                    "Previsão": "{:,.0f}"
                }),
                use_container_width=True,
                hide_index=True
